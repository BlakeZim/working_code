...
int main(int argc, char *argv[])
{
  ...
  imageWriter->SetFileName( argv[3] + std::string( ".mhd" ));
  ...
  // Set the spacing and origin of the input image - Defined from the deformation field

  ImageType::SpacingType spacingSetterInput;
  spacingSetterInput[0] = (2048.0/inputImage->GetBufferedRegion().GetSize()[0])*deformationField->GetSpacing()[0];
  spacingSetterInput[1] = (2048.0/inputImage->GetBufferedRegion().GetSize()[1])*deformationField->GetSpacing()[1];
  spacingSetterInput[2] = 1.0;

  ImageType::PointType originSetterInput;
  originSetterInput[0] = -(inputImage->GetBufferedRegion().GetSize()[0]*spacingSetterInput[0])/2; 
  originSetterInput[1] = -(inputImage->GetBufferedRegion().GetSize()[1]*spacingSetterInput[1])/2;
  originSetterInput[2] = 0.0; 

  inputImage->SetSpacing( spacingSetterInput );
  inputImage->SetOrigin( originSetterInput );
  inputImage->Update();
  
  std::cout << "Adjusted Input Image Origin = " << inputImage->GetOrigin() << std::endl;
  std::cout << "Adjusted Input Image Spacing = " << inputImage->GetSpacing() << std::endl;
  std::cout << "Adjusted Input Image size = " << inputImage->GetBufferedRegion().GetSize() << std::endl << std::endl;
...
//   //Define the resample filter
  ...
  resampleFilter->SetSize( inputImage->GetBufferedRegion().GetSize() );
  resampleFilter->SetOutputOrigin( originSetterInput );
  resampleFilter->SetOutputSpacing( spacingSetterInput );
  ...
  resampleFilter->SetNumberOfThreads( 6 );
  ...
  imageWriter->SetInput( finalImage );
